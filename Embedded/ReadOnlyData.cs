...
namespace Embedded {
  ...
  public readonly unsafe struct ReadOnlyData<T> where T : unmanaged {
    ...
    public ReadOnlyData<TOther> Recast<TOther>() where TOther : unmanaged {
      ...
      return new ReadOnlyData<TOther>((TOther*) _ptr, checked((int) Size / otherTSize));
    }
    ...
    public static implicit operator ReadOnlySpan<T>(ReadOnlyData<T> d)
      => new ReadOnlySpan<T>(d, d._length);

#endif

    private static string ToStringCache
    ...
    internal string GetDebugDisplay() {
      ...
      if (typeof(T) == typeof(char))
        return $"{t} @ 0x{ptrStr}: {System.Net.WebUtility.UrlEncode(new string((char*) TypedPointer, 0, _length))}";
      if (typeof(T) == typeof(byte) || typeof(T) == typeof(sbyte))
        return $"{t} @ 0x{ptrStr}: {System.Net.WebUtility.UrlEncode(new string((sbyte*) TypedPointer, 0, _length))}";
